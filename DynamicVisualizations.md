...
This CRAN Task View maintains a list of the dynamic graphics packages, for making animations and interactive plots. Static plots are not within the scope of this CRAN Task View, neither are graphics devices. Some packages that include only one or two functions to produce interactive plots or animations but are mostly focused on modeling methods or applications are mostly not included. If you think that a package is missing from this list, you can post an issue or directly add using a pull request in the [GitHub repository](https://github.com/cran-task-views/DynamicVisualizations).

The listed packages are grouped into general-purpose and those designed for specific applications: multivariate, temporal, maps, networks, and miscellaneous pacakges for specific plot types.
...
The `r pkg("r2d3")` package binds raw D3 JS codes to R data objects, allowiing D3 JS code to run with R-native data objects. This approach offers full customization of interactive details using JS but requires writing JS code.
...
- The `r pkg("rgl")` package provides 3D visualisations built with OpenGL or WebGL as the rendering backend.
